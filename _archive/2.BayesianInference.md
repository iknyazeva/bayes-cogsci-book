...
Notice how the uniform prior (equivalent to "zero prior observations") was updated by the data (7 successes, 3 failures) to produce a Beta posterior. This is an example of **conjugate updating**, which we will explore in depth in {doc}`4.ConjugatePriors`.
```
...
- For how conjugate priors simplify posterior computation (Beta-Binomial, Gamma-Poisson), see {doc}`4.ConjugatePriors`.
```
