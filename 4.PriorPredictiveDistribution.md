...
Run prior predictive simulations and compare flat, weakly informative, and informed priors in Google Colab:  
...
*Figure 4.11: Prior Predictive Audit. Notice how an extreme U-shaped prior $\text{Beta}(0.2, 0.2)$ assigns implausibly high predictive probability to boundary datasets where either 0 or 20 subjects succeed. While $k=0$ and $k=20$ are mathematically possible Binomial outcomes, assigning them dominant predictive weight in a typical cognitive task contradicts substantive domain expectations, exposing unreasonable prior assumptions before data collection:*
...
- {doc}`3.GenerativeModelsLikelihoodPrediction` — Generative models, likelihoods, and predictions
```
