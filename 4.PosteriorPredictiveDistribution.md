...
Using a $\text{Beta}(1, 1)$ (uniform) prior, the posterior is $\text{Beta}(10, 7)$. The posterior predictive probability that the patient will correctly recognize the **next** item is:

$$
P(y_{\text{new}} = 1 \mid y) = \frac{\alpha + \sum y_i}{\alpha + \beta + n} = \frac{10}{17} \approx 0.59
$$

This prediction accounts for uncertainty in $\theta$ — it is slightly pulled toward 0.5 compared to the raw proportion $9/15 = 0.60$ because of the remaining parameter uncertainty.
```
...
- The **posterior mean** is always a weighted average of the prior mean and data mean, with weights determined by effective sample sizes.
