...
```{tip}
**Why continuous distributions matter for cognitive science.** Many measurements in cognitive and neural science are inherently continuous: reaction times, EEG amplitudes, fMRI BOLD signals, psychophysical thresholds, and meal portion sizes. Continuous Bayesian models let us estimate means, variances, and rates from noisy observations — accounting for both what we know before the experiment (prior) and what the data tell us (likelihood). This part introduces the key continuous distributions and the priors that make Bayesian updating tractable.
```
...
```{admonition} Cognitive Science Example: Reaction Time Modeling
:class: tip
The ExGaussian distribution is one of the most widely used models for **reaction time (RT) data** in experimental psychology. The $\mu$ and $\sigma$ parameters capture the fast, Gaussian component of processing, while $1/\lambda$ (the exponential tail) captures slower, more variable cognitive processes such as attention lapses or decision difficulty. Fitting ExGaussian parameters to RT data can reveal which component of processing is affected by an experimental manipulation.
```
...
- A **probability** ($\theta \in [0, 1]$) needs a prior with support on $[0, 1]$, such as a Beta distribution.
- A **variance** ($\sigma^2 > 0$) needs a prior with positive support, such as Inverse-Gamma or Gamma.
- A **mean** ($\mu \in \mathbb{R}$) can use a Normal prior with support on the full real line.
- Mismatched support leads to invalid models — always check that the prior domain matches the parameter's range.
```

```{seealso}
- {doc}`4.NormalData` — Conjugate analysis for Normal data (known and unknown variance)
- {doc}`4.ExponentialData` — Gamma-Exponential conjugate model for waiting times
- {doc}`4.InverseGammaDistribution` — The Inverse-Gamma prior for variance parameters
- {doc}`4.NonInformativePriors` — Non-informative and weakly informative priors
- {doc}`4.JeffreysPrior` — Jeffreys prior: invariance under reparameterization
- {doc}`4.FisherInformation` — Fisher Information and its role in objective priors
```
