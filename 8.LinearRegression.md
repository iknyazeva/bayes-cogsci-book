...
1. **Aleatory Noise ($\sigma^2$)**: The irreducible observation noise in the physical process.
2. **Epistemic Parameter Uncertainty ($\boldsymbol{\phi}(\mathbf{x}^*)^T \mathbf{S}_N \boldsymbol{\phi}(\mathbf{x}^*)$)**: The uncertainty in our knowledge of the true regression plane, which shrinks to zero as $N \to \infty$.
...
- The posterior predictive variance explicitly decomposes into **epistemic uncertainty** (which vanishes with data) and **aleatory scatter** (which is irreducible).
