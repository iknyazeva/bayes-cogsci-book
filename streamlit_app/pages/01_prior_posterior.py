...
st.markdown("""
*Course: Bayesian Analysis of Empirical Data (Chapters 2–4)*  
Explore how prior beliefs are updated in light of observed binary evidence (e.g. opinion poll support, test accuracy, or policy uptake) using the conjugate **Beta-Binomial** model.
""")
...
preset = st.sidebar.selectbox(
    "Prior Preset",
    ["Custom", "Flat / Uniform (Beta(1, 1))", "Weakly Informative (Beta(2, 2))", "Skeptical (Beta(10, 10))", "Optimistic Prior (Beta(15, 5))"]
)
...
elif preset == "Skeptical (Beta(10, 10))":
...
n_trials = st.sidebar.slider("Sample Size ($N$)", min_value=1, max_value=200, value=30, step=1)
k_success = st.sidebar.slider("Observed Successes ($k$)", min_value=0, max_value=n_trials, value=18, step=1)

threshold = st.sidebar.slider("Threshold $p_0$ for $P(\\theta > p_0 \\mid \\text{data})$", min_value=0.0, max_value=1.0, value=0.5, step=0.05)
...
m2.metric("Observed Sample Rate ($k/N$)", f"{data_prop:.3f}")
m3.metric("Posterior Mean", f"{post_mean:.3f}", delta=f"{post_mean - prior_mean:+.3f}")
m4.metric(f"95% Credible Interval", f"[{ci_lower:.3f}, {ci_upper:.3f}]")
...
# Normalized likelihood curve for direct visual comparison
likelihood = stats.binom.pmf(k_success, n_trials, theta_grid)
# Normalize likelihood area to 1 for visual overlay
likelihood_norm = likelihood / np.trapz(likelihood, theta_grid) if np.trapz(likelihood, theta_grid) > 0 else likelihood
...
with tab1:
    ...
    idx_ci = (theta_grid >= ci_lower) & (theta_grid <= ci_upper)
    fig.add_trace(go.Scatter(
        x=theta_grid[idx_ci], y=post_pdf[idx_ci],
        mode='lines', name='95% Credible Region',
        fill='tozeroy', fillcolor='rgba(214, 39, 40, 0.35)',
    ...
    fig.update_layout(
        ...
        xaxis_title=r"Parameter $\theta$ (Probability of Success)",
    ...
    st.info(f"💡 **Posterior Inference Summary**: Given the prior $\\text{{Beta}}({alpha_prior}, {beta_prior})$ and observing ${k_success}$ successes out of ${n_trials}$ trials, there is a **{prob_above:.1%}** posterior probability that $\\theta > {threshold}$ (95% Credible Interval: [{ci_lower:.3f}, {ci_upper:.3f}]).")

with tab2:
    st.subheader("⏱️ Step-by-Step Bayesian Updating as Data Arrives")
    st.markdown("Watch how each individual binary observation shifts the posterior distribution.")
    
    sim_seed = st.number_input("Simulation Random Seed", value=42, step=1)
    np.random.seed(sim_seed)
    
    # Generate simulated arrival sequence
    ...
    fig_seq.add_trace(go.Scatter(x=theta_grid, y=curr_pdf, mode='lines', name=f'Updated Posterior at step {step} (k={curr_k}/{curr_n})', line=dict(color='#d62728', width=3), fill='tozeroy', fillcolor='rgba(214,39,40,0.2)'))
...
with tab3:
    ...
    for name, (a, b) in priors_comp.items():
        ...
        rows.append({"Prior": name, "Prior Mean": f"{a/(a+b):.3f}", "Posterior Mean": f"{p_mean:.3f}", "95% Credible Interval": f"[{lo:.3f}, {hi:.3f}]", f"P(θ > {threshold})": f"{p_above:.1%}"})
...
1. **Prior Weighting:** When sample size $N$ is small ($N=5$), how much does changing the prior shift the posterior mean? What happens when $N=150$?
2. **Prior Effective Sample Size:** How does a $\\text{Beta}(10, 10)$ prior act like having observed 20 prior respondents before the study begins?
3. **Decision Making:** If a social policy requires $P(\\theta > 0.5) > 95\\%$ to be approved, how does a skeptical prior alter the decision under modest sample evidence?
""")
